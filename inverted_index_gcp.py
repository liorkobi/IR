...
class InvertedIndex:
    def __init__(self, docs={}):
        self.dict_title= defaultdict(list)
    ...
    def get_posting_list(self, term,base_dir,bucket):
        # Return the posting list for the given term, possibly reading from disk
        if term in self.posting_locs:
                return self.read_a_posting_list(base_dir, term, bucket)
        else:
            # If the term isn't in posting_locs, return an empty list or handle appropriately
            return []
    ...
    def write_a_posting_list(b_w_pl, base_dir, bucket_name=None):
        ...
        with closing(MultiFileWriter(base_dir, bucket_id, bucket_name)) as writer:
            ...
            # path = f'{base_dir}/{bucket_id}_posting_locs.pickle'  # This will be "title/index.pkl"
    ...
    def read_index(base_dir, name, bucket_name=None):
        # Use forward slashes for paths, which is the standard in URLs and Unix-like systems.
        path = f'{base_dir}/{name}.pkl'  # This will be "title/index.pkl"
        bucket = None if bucket_name is None else get_bucket(bucket_name)
        with _open(path, 'rb', bucket) as f:
            index = pickle.load(f)
            print(f"Loaded object type: {type(index)}")  # Debug print
            return index
